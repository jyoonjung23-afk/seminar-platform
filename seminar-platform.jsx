...
const SUPABASE_ANON_KEY = 'sb_publishable_LNhjH630SHcpxEHWOht1jA_XVhx_...'; // 여기 수정

const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

export default function SeminarPlatform() {
  ...
  const generatePDF = async () => {
    ...
    try {
      const response = await fetch('https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js');
  ...
  const AdminMode = () => {
    ...
    const copyLink = () => {
      const link = `${window.location.origin}${window.location.pathname}?id=${generatedId}`;
              ...
              <div style={styles.resultBox}>
                ...
                <div style={styles.linkBox}>
                  <code style={styles.code}>
                    {window.location.origin}{window.location.pathname}?id={generatedId}
                  </code>
                </div>
